...
export class AlertDeliveryService {
  ...
  readonly #webhookSecret: Buffer | null;
  ...
  constructor(options: AlertDeliveryServiceOptions) {
    ...
    this.#webhookUrl = options.webhookUrl ? parseWebhookUrl(options.webhookUrl) : null;
    if (this.#webhookUrl && !options.webhookSecretFile) {
      throw new Error('alert webhook secret file is required when a webhook URL is configured');
    }
    this.#webhookSecret = this.#webhookUrl
      ? loadSecret(options.webhookSecretFile!)
      : null;
  }
  ...
  async list(limit = 50): Promise<{ enabled: boolean; deliveries: AlertDeliveryView[] }> {
  ...
  async retry(id: string, actorId: string): Promise<AlertDeliveryView> {
    ...
    const retried = await this.#store.retryAlertDelivery({
      ...
      audit: {
        ...
        targetId: id,
        detail: { previousAttempts: current.attempts, eventType: current.eventType },
      },
    });
    ...
    if (status.alerts.length > 0) {
      ...
      const queued = await this.#store.enqueueAlertDelivery({
        ...
        audit: {
          ...
          targetType: 'alert_delivery',
          targetId: eventId,
          detail: { source: payload.source, severity, reason: status.reason, fingerprint },
        },
      });
      ...
      try {
        ...
        const finished = await this.#store.finishAlertDelivery({
          ...
          audit: {
            ...
            detail: { attempts: delivery.attempts, eventType: delivery.eventType },
          },
        });

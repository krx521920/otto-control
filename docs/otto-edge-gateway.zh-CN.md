...
自定义 Secret Header 还会拒绝连接控制、报文长度/类型、代理转发、Cookie、请求追踪、
全部 `x-forwarded-*` 和 `x-otto-*` 内部 Header；Header 名大小写不影响判断。供应商确实需要新的认证 Header 时，
必须加入正向兼容测试，不能通过放宽上述协议边界解决。
...
响应完成回调与客户端数据流相互隔离：并发租约只释放一次；熔断成功、失败和取消回调，
...
限流器结果同样不被直接信任：`allowed` 必须是布尔值；允许结果的 `remaining` 必须是
非负安全整数且严格小于策略上限，并且重试时间必须为零；拒绝结果的剩余额度必须为零，
重试时间限制在 1 秒至 7 天，封禁标记只能是布尔值。任何畸形结果都按限流服务不可用
返回 503，不能因适配器类型错误而放行请求。
...
生命周期状态回退、篡改已经记录的激活/退役/撤销时间或恢复已撤销密钥都会 fail-closed。正常情况下每分钟检查一次；失败请求有退避，同一
未知 Key ID 也有强制发现频率限制，避免攻击者放大 Control 流量。Control 断开时只
能使用尚未过期的已验证 Keyring，超过签名有效期后停止验签。

`standby` 密钥只允许签署“将自身激活”的下一版 Keyring，不能签署业务策略或访问令牌；
`retired` 和 `revoked` 密钥一律不能验证在线业务策略或访问令牌。凭证自带的 `issuedAtMs`
由签名者控制，不能作为退役前签发的历史证明。`active` 是唯一可以签发并验证在线业务
对象的状态；确需验证历史对象时，必须使用退役前形成的不可篡改签发记录或透明日志。
...
结果落库、计费结算与 Hold 释放等 `waitUntil` 后台任务也进入同一个宽限窗口；HTTP 已结束
但后台任务未完成时不得报告 `drained`。进程最多跟踪 1,024 个后台任务，并记录当前积压、
峰值、失败数、溢出数及最近失败/溢出时间；失败会令 readiness 降级，达到上限或发生溢出会
令 readiness 返回不可用并停止接收新业务请求。上述聚合状态可通过受保护的
`GET /v1/operations/status` 查看，不包含任务内容或错误原文。

默认宽限期为 30 秒，可通过 `OTTO_EDGE_SHUTDOWN_GRACE_MS` 配置为 1 秒至 5 分钟。
该宽限期是整个关闭流程共享的绝对截止时间，同时约束监听器关闭、存量请求、后台任务、
计费协调器最终 flush 和 Redis `QUIT`。Redis 无法按期退出时强制断开；计费重试定时器会先
停止。只有所有步骤按期成功、后台任务历史失败/溢出计数均为零且排空期间没有新增异常，
才报告 `drained`。
任一关闭失败、超时或监听器未确认关闭都会强制断开连接并设置非零退出码，依靠耐久 journal
恢复，不能误报优雅关闭。第二次终止信号会立即强制断开连接。
排空期间仍允许存活、就绪和只读运维状态检查；运维状态只披露生命周期、活跃请求数和
后台任务聚合计数，不包含租户、账号、提示词、回复、上游地址、错误原文或密钥。
...
同一 endpoint 与公开模型的所有备用线路必须使用完全相同的计费模式和
`reserveUnits`。计费与免计费线路混用、或备用线路预留额度不同的签名策略会在 Control
签发及网关激活时被拒绝。这样故障转移既不能降级绕过计费，也不需要在一次请求中扩大
已有 Hold；网关按实际获准线路的同一额度发起预留。
...
解析器、限流器、可选共享计费协调器和必填的 `upstreamOriginPolicy`，并返回官方 Web
Service Worker 风格的 `fetch(request, context)` 入口。ESA 引导层必须传入原生执行上下文，
...
速率控制应组合 ESA 原生流量规则和一个支持原子消费的限流器。多节点计费事件现已汇入
Control 的 PostgreSQL 有序聚合队列；每个稳定节点身份使用独立签名密钥和单调序列，不能
共享本地 journal 或把 journal 放到 NFS/SMB。ESA Worker 尚需根据其实际稳定节点身份模型
完成运行时接线验收；无法提供稳定节点身份和耐久序列时不得伪装成 Node 聚合模式。
...
- fast-check：属性测试与可复现 Fuzz，每轮生成 1,800 组限流、并发租约、熔断边界、窗口、畸形令牌、
  签名变异、协议和认证头输入；
- Stryker + Vitest Runner：对 `gateway.ts`、`aliyun-esa.ts`、
  `aliyun-esa-rollout.ts`、`background-tasks.ts`、`server-lifecycle.ts`、`control-keyring-verifier.ts`、
...
提示文案的 `StringLiteral` 变异。最低门槛为 80%；2026-08-14 的最近一次全量正式
基线总分为 84.63%，已覆盖代码为 85.30%。该基线包含 4,601 个有效变异：3,886 个
被杀死、8 个超时、671 个存活、36 个无覆盖。其中 ESA 灰度与回滚状态机为 79.29%、
ESA 运行时适配器为 100.00%、后台任务跟踪器为 85.33%、Node 排空编排为 79.87%、
Redis 分布式限流为 86.29%、限流与输入校验层为 84.38%、公钥轮换模块为 78.96%、
策略自动同步器为 87.11%、Control 签发服务为 82.77%、网关核心为 84.41%、用量解析器为 80.83%、
单机计费协调器为 80.79%、单机并发限制器为 100%、优雅排空状态机为 100%、
Node HTTP 适配器为 100%、Node HTTP 资源边界为 100%、上游响应限制配置为 100%、
上游熔断器为 96.06%、协议层为 79.25%、本地请求限制配置为 100%、本地上游 Origin
与凭据绑定策略为 98.45%。
...
限流结果接入门禁的本次行范围变异复验为 100%。
ESA 灰度状态机、公钥轮换、协议层和 Node 排空编排的单文件得分仍低于总体门槛，
应继续补强，不能用总体分数掩盖薄弱模块；
正式门禁仍以全部纳入范围的总分为准。
...
- 在真实 ESA 账号落地 Secret、KV、WAF、域名、TLS 和 Terraform 资源，接通受控 OpenAPI
  发布驱动；灰度状态机、自动回滚、断点恢复和 fail-closed IaC 基线已实现；
- 在真实 ESA 节点完成 Keyring 双钥重叠、紧急撤销和回滚演练；相关受限演练工具已实现；

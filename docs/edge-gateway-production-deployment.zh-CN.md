...
正式镜像必须使用不可变仓库摘要。升级脚本先启动使用独立状态卷的隔离预检实例
（Compose 服务名保留为 `edge-gateway-canary`），确认它能
加载真实签名策略、连接 Control/TLS Redis 并通过 `/readyz`，才切换生产实例。生产实例
未通过就绪检查时会自动恢复旧镜像：

该实例不接收真实用户流量，因此这是单服务器蓝绿切换的发布前验证，不应作为“真实流量
灰度”证据。需要按 5%、25%、100% 分批承接真实流量时，使用 ESA 灰度状态机或外部负载
均衡器，并归档每阶梯错误率、P95、计费就绪和回滚证据。
...
升级状态只保存镜像摘要，不保存令牌或密钥。若隔离预检失败，生产实例保持不变；若自动
回滚也失败，脚本退出非零并要求人工接管。
